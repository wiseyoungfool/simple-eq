...
ChainSettings getChainSettings(juce::AudioProcessorValueTreeState& apvts) {
    ...
    //settings.lowCutSlope = static_cast<Slope>(settings.lowCutSlope);
    //settings.highCutSlope = static_cast<Slope>(settings.highCutSlope);
...
SimpleEQAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Quality", "Peak Quality", juce::NormalisableRange<float>(.1f, 10.f, 0.05f, 0.25f), 1.f));

    //Create string array of slope values: 12, 24, 36 & 48 db per octave
